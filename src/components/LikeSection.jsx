...
const LikeSection = ({ id }) => {
  ...
  useEffect(() => {
    onSnapshot(collection(db, "tasks", id, "likes"), (snapshot) => {
  ...
  const likeTask = async () => {
    if (hasLiked) {
      await deleteDoc(doc(db, "tasks", id, "likes", session?.user?.uid));
    } else {
      await setDoc(doc(db, "tasks", id, "likes", session?.user?.uid), {
